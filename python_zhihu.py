...
class ZhiHu():
    ...
    def get_question_followers(self,url):
        global _session
        #r=_session.get(url,headers=header_data,verify=True)
        i = 1
        for x in range(20,3600,20):
            print(x)
            data={'start':'0','offset':str(x),'_xsrf':self.xsrf}
                  
            content=_session.get(url,headers=header_data,data=data,timeout=10).text
            #content=requests.post(url,headers=header_data,data=data,timeout=10).text
           #print(content)
